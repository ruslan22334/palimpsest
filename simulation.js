...
'use strict';
const S=typeof Spellcraft!=='undefined'?Spellcraft:require('./spellcraft.js'),F=A.fortifications,E=A.ecology,G=A.mages;
const {SIZE,TILE,MANA,SHRINES,clamp,dist,cost,canPay,pay,terrainReaction,biome,manaRates}=A;
class Simulation{
constructor(state){this.s=state;this.random=A.rng(state.seed+Math.floor(state.time*10));this.events=[];this.particles=[];this.projectiles=[];this.fields=[];this.rings=[];this.delayed=[];this.floaters=[];this.cooldown=0;this.dashCooldown=0;this.invincible=0;this.haste=0;this.burnAura=0;this.shake=0;this.autoSave=0;this.rates={};this.drains={};this.drainLinks=[];this.channels=new Map();this.mageBrains=new Map();this.exposure=A.daylight.exposure(state.tiles,state.player,state.time);state.nextEnemyId=Math.max(state.nextEnemyId,1+Math.max(0,...state.enemies.map(e=>e.id)));this.active=true;this.moving=false;this.terrainTimers=state.terrainTimers;this.pulse=0;if(!state.worldPopulated&&!state.won){this.populate();state.worldPopulated=true;}if(!state.ecology.populated)this.populateEcology();if(!state.magesPopulated)this.populateMages();this.reveal();}
...
blocked(x,y,r=11){return [[-r,-r],[r,-r],[-r,r],[r,r]].some(([dx,dy])=>F.solid(this.tile(x+dx,y+dy)));}
move(e,dx,dy,r=11){const steps=Math.max(1,Math.ceil(Math.hypot(dx,dy)/8));for(let i=0;i<steps;i++){if(!this.blocked(e.x+dx/steps,e.y,r))e.x+=dx/steps;if(!this.blocked(e.x,e.y+dy/steps,r))e.y+=dy/steps;}e.x=clamp(e.x,TILE+15,(SIZE-1)*TILE-15);e.y=clamp(e.y,TILE+15,(SIZE-1)*TILE-15);}
wallCenter(target,form='bolt',range=1){const p=this.s.player,dx=target.x-p.x,dy=target.y-p.y,d=Math.hypot(dx,dy)||1,reach={bolt:340,burst:100,field:260,self:85}[form]*range,length=Math.min(Math.max(d,80),reach);return {x:p.x+(dx||(!dy?1:0))/d*length,y:p.y+dy/d*length};}
wallPlan(spell,target){let c=this.wallCenter(target,spell.form,spell.mod==='reach'?1.65:1);if(spell.form==='bolt'){const hit=this.traceWall(this.s.player.x,this.s.player.y,c.x,c.y);if(hit)c={x:hit.x,y:hit.y};}return {...F.plan(this.s,c),center:c};}
placeWall(center){const plan=F.plan(this.s,center);for(const i of plan.cells){this.setTerrain(i,'barrier');this.s.walls[i]={hp:80,maxHp:80,material:'earth'};this.burstParticles((i%SIZE+.5)*TILE,(Math.floor(i/SIZE)+.5)*TILE,'#bda581',10,60);}if(plan.cells.length){this.observe('reaction:barrier');this.ring(center.x,center.y,55,'#bda581');}return plan.cells.length;}
strikeWall(index,node){const wall=this.s.walls[index];if(!wall)return false;const before=wall.material,result=F.strike(wall,node);if(result.damage>0||before!==wall.material){this.observe(before!==wall.material?'wall:'+wall.material:'wall:damage');this.burstParticles((index%SIZE+.5)*TILE,(Math.floor(index/SIZE)+.5)*TILE,'#bda581',3,40);if(wall.hp<=0)this.setTerrain(index,'stone');return true;}return false;}
traceWall(x,y,xx,yy){const d=Math.hypot(xx-x,yy-y),steps=Math.max(1,Math.ceil(d/4));for(let i=0;i<=steps;i++){const px=x+(xx-x)*i/steps,py=y+(yy-y)*i/steps;if(F.solid(this.tile(px,py)))return {x:px,y:py,index:Math.floor(py/TILE)*SIZE+Math.floor(px/TILE)};}return null;}
breakObstacle(e,dt){e.wallCool=Math.max(0,(e.wallCool||0)-dt);if(e.wallCool>0||e.stun>0||e.fear>0)return false;const p=this.s.player,d=Math.hypot(p.x-e.x,p.y-e.y)||1;const hit=this.traceWall(e.x,e.y,e.x+(p.x-e.x)/d*48,e.y+(p.y-e.y)/d*48);if(!hit||!this.s.walls[hit.index])return false;this.strikeWall(hit.index,{element:'earth',weight:e.kind==='boss'?2:e.kind==='brute'?1.4:.7});e.wallCool=1.1;this.ring(e.x,e.y,23,'#c7a184');return true;}
spawn(kind,x,y,guard=-1){if(this.blocked(x,y,18)){let found=false;for(let r=1;r<=5&&!found;r++)for(let dy=-r;dy<=r&&!found;dy++)for(let dx=-r;dx<=r&&!found;dx++){const xx=x+dx*TILE,yy=y+dy*TILE;if(!this.blocked(xx,yy,18)){x=xx;y=yy;found=true;}}if(!found){x=this.s.checkpoint.x;y=this.s.checkpoint.y;}}const defs={mage:[85,64,10],leech:[46,66,5],wisp:[34,76,7],crawler:[52,59,11],archer:[42,48,8],brute:[115,42,17],shade:[65,89,12],sentinel:[95,53,13]};const [hp,speed,damage]=defs[kind]||defs.wisp;let e={id:this.s.nextEnemyId++,kind,x,y,hp,maxHp:hp,speed,damage,cool:1+this.random(),slow:0,burn:0,stun:0,guard,homeX:x,homeY:y,phase:this.random()*6.28};if(this.s.difficulty==='gentle'){e.hp*=.8;e.maxHp=e.hp;e.damage*=.65;}if(this.s.difficulty==='hard'){e.hp*=1.25;e.maxHp=e.hp;e.damage*=1.3;}if(kind==='mage')Object.assign(e,G.initial());if(kind==='leech')e.drainElement='water';this.s.enemies.push(e);return e;}
...
populateEcology(){const s=this.s;for(const [i,site]of E.SITES.entries()){let center=null;for(let r=0;r<=4&&!center;r++)for(let dy=-r;dy<=r&&!center;dy++)for(let dx=-r;dx<=r&&!center;dx++){const x=site.x+dx*TILE,y=site.y+dy*TILE;if(!this.blocked(x,y,18)&&!['lava','fire'].includes(this.tile(x,y))&&dist({x,y},s.player)>180&&dist({x,y},s.checkpoint)>220)center={x,y};}if(!center)continue;s.ecology.anomalies.push({id:i+1,...center,element:site.element,neutralized:false});if(!s.won)for(const sign of [-1,1]){let placed=false;for(let r=0;r<=3&&!placed;r++)for(let dy=-r;dy<=r&&!placed;dy++)for(let dx=-r;dx<=r&&!placed;dx++){const x=center.x+sign*90+dx*TILE,y=center.y+90+dy*TILE;if(this.blocked(x,y,18)||['lava','fire'].includes(this.tile(x,y))||dist({x,y},s.player)<180||dist({x,y},s.checkpoint)<300)continue;const e=this.spawn('leech',x,y);e.drainElement=i%2?'life':'water';placed=true;}}}s.ecology.populated=true;}
affectAnomalies(x,y,r,node){if(node.kind!=='aspect')return;for(const a of this.s.ecology.anomalies){if(a.neutralized||Math.hypot(a.x-x,a.y-y)>r+18)continue;if(E.TYPES[a.element].counter!==node.element){this.ring(a.x,a.y,25,'#c79ac8');continue;}a.neutralized=true;this.channels.delete('anomaly:'+a.id);if(!this.effectOwner)this.s.shards+=3;this.observe('ecology:counter:'+a.element);this.float(a.x,a.y-25,this.effectOwner?'Угасло':'Угасло · +3','#b9ddbc');this.ring(a.x,a.y,70,'#b9ddbc');this.emit('save');}}
updateEcology(dt){
 this.drains={};this.drainLinks=[];const live=new Set();
 const sources=[...this.s.ecology.anomalies.filter(a=>!a.neutralized).map(a=>({body:a,key:'anomaly:'+a.id,kind:'anomaly',element:a.element,radius:125,delay:1.2,rate:7})),...this.s.enemies.filter(e=>e.kind==='leech'&&e.hp>0).map(e=>({body:e,key:'leech:'+e.id,kind:'leech',element:e.drainElement,radius:160,delay:.85,rate:5}))];
 for(const p of [this.s.player,...this.s.enemies.filter(e=>e.kind==='mage'&&e.hp>0)])for(const source of sources){
  const {body,kind,element,radius,delay,rate}=source,isPlayer=p===this.s.player,key=source.key+(isPlayer?'':':mage:'+p.id);live.add(key);
  const channel=this.channels.get(key)||{charge:0,cool:0};channel.cool=Math.max(0,(channel.cool||0)-dt);this.channels.set(key,channel);
  const eligible=dist(body,p)<=radius&&!this.traceWall(body.x,body.y,p.x,p.y)&&!(body.stun>0||body.fear>0||channel.cool>0);
  const amount=E.advance(channel,eligible,dt,p.mana[element],rate,delay);
  if(amount>0){p.mana[element]=Math.max(0,p.mana[element]-amount);if(isPlayer){this.drains[element]=(this.drains[element]||0)+amount/dt;this.observe('ecology:drain:'+element);}}
  if(eligible&&p.mana[element]>0)this.drainLinks.push({x:body.x,y:body.y,tx:p.x,ty:p.y,targetId:isPlayer?null:p.id,element,kind,active:amount>0,progress:channel.charge/delay});
 }
 for(const key of this.channels.keys())if(!live.has(key))this.channels.delete(key);
}

populateMages(){
 const s=this.s;
 if(!s.won)for(const site of G.SITES){let placed=false;
  for(let r=0;r<=5&&!placed;r++)for(let dy=-r;dy<=r&&!placed;dy++)for(let dx=-r;dx<=r&&!placed;dx++){
   const x=site.x+dx*TILE,y=site.y+dy*TILE;
   if(this.blocked(x,y,18)||['lava','fire'].includes(this.tile(x,y))||dist({x,y},s.player)<350||dist({x,y},s.checkpoint)<400)continue;
   const e=this.spawn('mage',x,y);Object.assign(e,G.initial(site.school));placed=true;
  }
 }
 s.magesPopulated=true;
}
castFrom(caster,target,recipe){
 const program=S.compile(recipe);
 if(caster===this.s.player)return this.cast(target,false,recipe);
 if(!program||caster.kind!=='mage'||caster.hp<=0||caster.stun>0||caster.fear>0||!G.allowed(caster,recipe)||!Number.isFinite(target?.x)||!Number.isFinite(target?.y))return false;
 const brain=this.mageBrain(caster);if(brain.recovery>0||!pay(caster.mana,program.cost))return false;
 brain.recovery=Math.max(1.4,program.cooldown);this.withOwner(caster,()=>this.launchSpell(caster,program.spell,target));
 this.observe('mage:cost');this.emit('sound',{name:'cast',element:program.spell.element});return true;
}
mageBrain(e){let b=this.mageBrains.get(e.id);if(!b){b={cast:null,recovery:1,search:0,path:[],x:e.x,y:e.y,state:'Наблюдает',rates:{}};this.mageBrains.set(e.id,b);}return b;}
magePath(e,recipe){
 const required=S.cost(recipe),start=Math.floor(e.y/TILE)*SIZE+Math.floor(e.x/TILE),seen=new Set([start]),queue=[{id:start,path:[]}];let best=null,score=0;
 for(let i=0;i<queue.length;i++){const node=queue[i],x=node.id%SIZE,y=Math.floor(node.id/SIZE),p={x:(x+.5)*TILE,y:(y+.5)*TILE};
  const rates=manaRates(this.s.tiles,p,false,[],this.s.time);let value=0;
  for(const [id,cost]of Object.entries(required))if(e.mana[id]<cost)value+=rates[id]/(1+node.path.length*.35);
  if(value>score){score=value;best=node.path;}
  if(node.path.length>=10)continue;
  for(const [dx,dy]of [[1,0],[-1,0],[0,1],[0,-1]]){const xx=x+dx,yy=y+dy,id=yy*SIZE+xx;
   if(xx<=1||yy<=1||xx>=SIZE-2||yy>=SIZE-2||seen.has(id))continue;seen.add(id);
   const px=(xx+.5)*TILE,py=(yy+.5)*TILE;if(this.blocked(px,py,11)||['lava','fire'].includes(this.tile(px,py)))continue;
   queue.push({id,path:[...node.path,{x:px,y:py}]});
  }
 }
 return best;
}
mageWalk(e,target,dt){if(e.root>0||e.stun>0)return;const d=dist(e,target);if(d<3)return;const terrain=this.tile(e.x,e.y),speed=e.speed*(e.slow>0?.4:1)*(terrain==='water'?.58:terrain==='mud'?.65:terrain==='tree'?.8:1),step=Math.min(d,speed*dt);
 const dx=(target.x-e.x)/d*step,dy=(target.y-e.y)/d*step;if(!['fire','lava'].includes(this.tile(e.x+dx,e.y+dy)))this.move(e,dx,dy);
}
updateMage(e,dt){
 const b=this.mageBrain(e),p=this.s.player,school=G.SCHOOLS[e.school];
 const moving=Math.hypot(e.x-b.x,e.y-b.y)>.05;b.x=e.x;b.y=e.y;
 b.rates=manaRates(this.s.tiles,e,moving,[],this.s.time);for(const id of S.IDS)e.mana[id]=Math.min(e.cap,e.mana[id]+b.rates[id]*dt);
 b.recovery=Math.max(0,b.recovery-dt);b.search-=dt;
 if(e.fear>0||e.stun>0){b.cast=null;b.state='Сбит';if(e.fear>0)this.flee(e,dt);return;}
 if(dist(e,p)>430||this.s.won){b.cast=null;b.state='Собирает ману';if(dist(e,{x:e.homeX,y:e.homeY})>30)this.mageWalk(e,{x:e.homeX,y:e.homeY},dt);return;}
 if(b.cast){b.cast.left-=dt;b.state='Готовит заклинание';if(b.cast.left<=0){const cast=b.cast;b.cast=null;if(cast.spell.form==='self'||!this.traceWall(e.x,e.y,cast.target.x,cast.target.y))this.castFrom(e,cast.target,cast.spell);else b.recovery=.4;}return;}
 if(b.recovery>0){b.state='Восстанавливает стойку';return;}
 let recipe=e.hp<e.maxHp*.7&&e.shield<8&&canPay(e.mana,S.cost(G.SHIELD))?G.SHIELD:dist(e,p)<115&&school.close?school.close:school.attack;
 if(!canPay(e.mana,S.cost(recipe))&&school.fallback&&canPay(e.mana,S.cost(school.fallback)))recipe=school.fallback;
 if(!canPay(e.mana,S.cost(recipe))){b.state='Ищет источник';if(b.search<=0){b.path=this.magePath(e,recipe);b.search=1;}
  if(b.path?.length){if(dist(e,b.path[0])<5)b.path.shift();if(b.path[0])this.mageWalk(e,b.path[0],dt);}
  else if(b.path===null&&S.sequence(recipe).includes('air')){const a=this.s.time*.7+e.id;this.mageWalk(e,{x:e.homeX+Math.cos(a)*70,y:e.homeY+Math.sin(a)*70},dt);}
  else b.state=b.path===null?'Источник недоступен':'Собирает ману';return;
 }
 if(recipe.form!=='self'&&(dist(e,p)>285||this.traceWall(e.x,e.y,p.x,p.y))){b.state='Меняет позицию';this.mageWalk(e,p,dt);this.breakObstacle(e,dt);return;}
 b.cast={spell:recipe,target:recipe.form==='bolt'?{x:p.x,y:p.y}:{x:e.x,y:e.y},left:1,total:1};b.state='Готовит заклинание';this.ring(e.x,e.y,28,school.color);
}
...
cast(target,free=false,override=null){const s=this.s,p=s.player,program=S.compile(override||s.spells[s.selected]);if(!program||!target||!Number.isFinite(target.x)||!Number.isFinite(target.y))return false;const spell=program.spell;if(!free&&(this.cooldown>0||p.stun>0))return false;if(!free&&program.nodes.every(n=>n.id==='barrier')){const plan=this.wallPlan(spell,target);if(!plan.cells.length){this.notice(plan.reason);this.cooldown=.25;return false;}target=plan.center;}const c=program.cost;if(!free&&!pay(p.mana,c)){this.cooldown=.25;this.notice('Не хватает маны: '+Object.entries(c).filter(([k,v])=>p.mana[k]<v).map(([k,v])=>K.view(this.s.knowledge,MANA.find(m=>m.id===k)).name+' '+Math.ceil(v-p.mana[k])).join(', '));this.emit('sound',{name:'dry'});return false;}
if(!free){this.cooldown=program.cooldown;s.stats.casts++;this.emit('sound',{name:'cast',element:spell.element});}
this.launchSpell(p,spell,target,free);return true;}
launchSpell(p,spell,target,free=false){const enemy=p!==this.s.player;const dx=target.x-p.x,dy=target.y-p.y,d=Math.hypot(dx,dy)||1,range=spell.mod==='reach'?1.65:1,power=(spell.mod==='power'?1.65:1)*(!enemy&&this.has('lens')?1.2:1),color=this.spellColor(spell);
if(spell.form==='self')this.executeSelf(spell,target,power,range,p);
else if(spell.form==='bolt'){this.projectiles.push({x:p.x,y:p.y,vx:dx/d*440,vy:dy/d*440,life:Math.max(.06,Math.min(d/440,.85*range)),r:7,spell:{...spell},power,color,enemy,attacker:enemy?p:null,trail:0});}
else if(spell.form==='burst'){this.area(p.x,p.y,125*range,spell,27*power,this.wallCenter(target,'burst',range),enemy?p:null);this.ring(p.x,p.y,125*range,color);this.burstParticles(p.x,p.y,color,35,170);}
else{const reach=Math.min(d,260*range),x=p.x+dx/d*reach,y=p.y+dy/d*reach;this.fields.push({x,y,r:84*range,life:4,tick:0,spell:{...spell},power,color,attacker:enemy?p:null});this.ring(x,y,84*range,color);}
...
this.s.tiles[index]=next;delete this.s.walls[index];delete this.terrainTimers[index];delete this.s.terrainCredit[index];
...
if(next==='fire'&&!this.effectOwner)this.s.terrainCredit[index]=true;if(!this.effectOwner)this.s.stats.transforms++;this.emit('terrain',{index});return true;
}
transform(x,y,r,input,aim){const nodes=typeof input==='string'?[{kind:'aspect',element:input,weight:1}]:S.compile(input)?.nodes;if(!nodes)return;
for(const node of nodes)if(!(node.id==='barrier'&&aim===false))this.transformNode(x,y,r,node,aim);
}
transformNode(x,y,r,node,aim){this.affectAnomalies(x,y,r,node);if(node.id==='barrier'){this.placeWall(aim||{x,y});return;}const tx=Math.floor(x/TILE),ty=Math.floor(y/TILE),n=Math.ceil(r/TILE);let changed=false;
...
const index=yy*SIZE+xx,old=this.s.tiles[index];if(old==='barrier'){this.strikeWall(index,node);continue;}let next=old;
...
withOwner(owner,action){const previous=this.effectOwner;this.effectOwner=owner;try{return action();}finally{this.effectOwner=previous;}}
area(x,y,r,spell,damage,aim,owner=null){this.withOwner(owner,()=>{this.transform(x,y,r*.7,spell,aim);if(owner){if(dist(this.s.player,{x,y})<r)this.hitPlayer(damage,spell,owner,x,y);}else{for(const e of [...this.s.enemies])if(e.hp>0&&Math.hypot(e.x-x,e.y-y)<r)this.hit(e,damage,spell,x,y);const b=this.s.boss;if(b&&b.hp>0&&Math.hypot(b.x-x,b.y-y)<r+25)this.hit(b,damage,spell,x,y);}});}
hitPlayer(damage,spell,owner,x,y){if(this.invincible>0||this.s.won)return;this.playerImpact=0;this.withOwner(owner,()=>this.hit(this.s.player,damage,spell,x,y));const total=this.playerImpact;this.playerImpact=null;if(total>0)this.hurt(total,owner);}

reactionSelf(node,spell,power,range,target){if(node.id==='barrier'){this.placeWall(this.wallCenter(target||this.s.player,'self',range));return;}const p=this.s.player,pw=power*node.weight,before={hp:p.hp,shield:p.shield},values={steam:[8,100,12],quench:[14,95,10],frost:[18,95,8],mire:[20,110,6],bark:[30,95,8],eclipse:[0,150,10]}[node.id];
...
reactionHit(e,node,damage,spell,ox,oy){if(node.id==='barrier')return;if(e.hp<=0)return;const body=C.profile(e),id=node.id,weight=node.weight;let n=0;
...
if(e===this.s.player){const n=C.damage({...e,hp:e.hp+e.shield},amount,element);this.playerImpact+=n;return Math.min(n,Math.max(0,e.hp-Math.max(0,this.playerImpact-n-e.shield)));}
const raw=C.damage({...e,hp:e.hp+(e.shield||0)},amount,element),blocked=Math.min(e.shield||0,raw);if(blocked)e.shield-=blocked;const n=raw-blocked;if(n<=0)return 0;e.hp-=n;
this.emit('damage',{target:'enemy',targetId:e.id,element,amount:n,source});
const credited=!this.effectOwner&&(source==='spell'||source==='aura'||source==='burn'&&e.playerBurn||source==='environment'&&this.s.terrainCredit[Math.floor(e.y/TILE)*SIZE+Math.floor(e.x/TILE)]);
...
if(e.kind==='mage'){const brain=this.mageBrains.get(e.id);if(brain?.cast){brain.cast=null;brain.recovery=.8;this.float(e.x,e.y-42,'Сбито','#dce8ca');this.observe('mage:interrupt');}}
if(e.kind==='leech'){const channel=this.channels.get('leech:'+e.id);if(channel?.charge>0)this.observe('ecology:interrupt');for(const key of this.channels.keys())if(key==='leech:'+e.id||key.startsWith('leech:'+e.id+':'))this.channels.set(key,{charge:0,cool:.9});}
...
if(spell.mod==='chain'&&e!==this.s.player){const targets=this.s.enemies.filter(t=>t.id!==e.id&&t.hp>0&&dist(t,e)<160).slice(0,2);for(const t of targets){this.hit(t,damage*.6,{...program.spell,mod:'plain'},e.x,e.y);this.ring(t.x,t.y,24,'#f0d995');}}
}
...
if(element==='blood'&&n>0){const p=this.effectOwner||this.s.player,heal=Math.min(p.maxHp-p.hp,n*.15);if(heal>0){p.hp+=heal;this.float(p.x,p.y-26,'+'+Math.round(heal),'#ed8295');this.observe('status:blood');}}
...
p.slow=Math.max(0,(p.slow||0)-dt);p.stun=Math.max(0,(p.stun||0)-dt);p.root=Math.max(0,(p.root||0)-dt);p.wet=Math.max(0,(p.wet||0)-dt);if(p.burn>0){p.burn=Math.max(0,p.burn-dt);if(this.tile(p.x,p.y)==='water')p.burn=0;else this.hurt(2);}
let dx=input.x||0,dy=input.y||0;if(p.stun>0||p.root>0)dx=dy=0;let d=Math.hypot(dx,dy);const oldx=p.x,oldy=p.y;if(d){let speed=155*(p.slow>0?.55:1)*(this.has('stride')?1.12:1)*(this.haste>0?1.6:1);if(this.tile(p.x,p.y)==='water')speed*=.58;if(this.tile(p.x,p.y)==='tree')speed*=.8;if(this.tile(p.x,p.y)==='mud')speed*=.65;this.move(p,dx/d*speed*dt,dy/d*speed*dt);}this.moving=Math.hypot(oldx-p.x,oldy-p.y)>.1;
...
for(const b of [...this.projectiles].reverse()){if(!this.projectiles.includes(b))continue;b.life-=dt;const collision=this.traceWall(b.x,b.y,b.x+b.vx*dt,b.y+b.vy*dt);b.x=collision?collision.x:b.x+b.vx*dt;b.y=collision?collision.y:b.y+b.vy*dt;let remove=b.life<=0||!!collision;if(collision&&b.enemy&&!b.spell&&this.s.walls[collision.index])this.strikeWall(collision.index,{element:'earth',weight:b.damage/22});
if(b.enemy){if(!collision&&Math.hypot(b.x-p.x,b.y-p.y)<b.r+12){if(b.spell)this.hitPlayer(24*b.power,b.spell,b.attacker,b.attacker.x,b.attacker.y);else this.hurt(b.damage,b.attacker);remove=true;}if(remove&&b.spell)this.withOwner(b.attacker,()=>{this.transform(b.x,b.y,35,b.spell);this.ring(b.x,b.y,25,b.color);});}
else{b.trail=(b.trail||0)+dt;if(b.trail>.025){b.trail=0;this.burstParticles(b.x,b.y,b.color,1,8);}const targets=s.boss?[...s.enemies,s.boss]:s.enemies;for(const e of targets){if(!collision&&e.hp>0&&Math.hypot(b.x-e.x,b.y-e.y)<(e.kind==='boss'?35:e.kind==='brute'?22:16)+b.r){this.hit(e,24*b.power,b.spell,p.x,p.y);remove=true;break;}}if(!collision&&s.ecology.anomalies.some(a=>!a.neutralized&&Math.hypot(a.x-b.x,a.y-b.y)<18+b.r))remove=true;if(remove){this.transform(b.x,b.y,35,b.spell);this.ring(b.x,b.y,25,b.color);}}
if(remove){const index=this.projectiles.indexOf(b);if(index>=0)this.projectiles.splice(index,1);}}
for(let i=this.fields.length-1;i>=0;i--){const f=this.fields[i];f.life-=dt;f.tick-=dt;if(f.tick<=0){f.tick=.65;this.area(f.x,f.y,f.r,f.spell,10*f.power,f.built?false:undefined,f.attacker||null);f.built=true;this.burstParticles(f.x+(this.random()-.5)*f.r,f.y+(this.random()-.5)*f.r,f.color,3,20);}if(f.life<=0)this.fields.splice(i,1);}
for(const e of s.enemies){if(e.hp<=0)continue;e.flash=Math.max(0,(e.flash||0)-dt);this.updateAfflictions(e,dt);if(e.hp<=0)continue;e.slow=Math.max(0,e.slow-dt);e.stun=Math.max(0,e.stun-dt);if(e.kind==='mage'){this.updateMage(e,dt);if(this.burnAura>0&&dist(e,p)<92)this.damageEnemy(e,14*dt,'fire',undefined,'aura');continue;}if(e.fear>0){this.flee(e,dt);continue;}if(e.stun>0)continue;const distance=dist(e,p),aggro=e.guard>=0?600:350;if(distance>aggro){if(!(e.root>0)&&Math.hypot(e.x-e.homeX,e.y-e.homeY)>20){const angle=Math.atan2(e.homeY-e.y,e.homeX-e.x);this.move(e,Math.cos(angle)*e.speed*.4*dt,Math.sin(angle)*e.speed*.4*dt);}continue;}e.cool-=dt;const a=Math.atan2(p.y-e.y,p.x-e.x);let speed=e.speed*(e.slow>0?.4:1);if(e.stun>0||e.root>0)speed=0;
if(e.kind==='leech'){if(distance>115||this.traceWall(e.x,e.y,p.x,p.y))this.move(e,Math.cos(a)*speed*dt,Math.sin(a)*speed*dt);this.breakObstacle(e,dt);}
else if(e.kind==='archer'||e.kind==='wisp'||e.kind==='sentinel'){if(distance>190)this.move(e,Math.cos(a)*speed*dt,Math.sin(a)*speed*dt);else if(distance<110)this.move(e,-Math.cos(a)*speed*dt,-Math.sin(a)*speed*dt);if(e.cool<=0){this.enemyShot(e,a,e.kind==='sentinel'?190:140);if(e.kind==='sentinel'){this.enemyShot(e,a+.2,170);this.enemyShot(e,a-.2,170);}e.cool=e.kind==='wisp'?2.2:1.7;}}
else{if(distance>24){const ox=e.x,oy=e.y;this.move(e,Math.cos(a)*speed*dt,Math.sin(a)*speed*dt,e.kind==='brute'?16:11);if(Math.hypot(e.x-ox,e.y-oy)<.1)this.move(e,Math.cos(a+1.2)*speed*dt,Math.sin(a+1.2)*speed*dt);}this.breakObstacle(e,dt);if(distance<34&&e.cool<=0&&!this.traceWall(e.x,e.y,p.x,p.y)){this.hurt(e.damage,e);e.cool=1.1;}}
...
s.enemies=s.enemies.filter(e=>e.hp>0);for(const id of this.mageBrains.keys())if(!s.enemies.some(e=>e.id===id))this.mageBrains.delete(id);this.updateEcology(dt);
