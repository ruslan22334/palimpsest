...
'use strict';
const S=typeof Spellcraft!=='undefined'?Spellcraft:require('./spellcraft.js'),F=A.fortifications;
...
cast(target,free=false,override=null){const s=this.s,p=s.player,program=S.compile(override||s.spells[s.selected]);if(!program||!target||!Number.isFinite(target.x)||!Number.isFinite(target.y))return false;const spell=program.spell;if(!free&&this.cooldown>0)return false;if(!free&&program.nodes.every(n=>n.id==='barrier')){const plan=this.wallPlan(spell,target);if(!plan.cells.length){this.notice(plan.reason);this.cooldown=.25;return false;}target=plan.center;}const c=program.cost;if(!free&&!pay(p.mana,c)){this.cooldown=.25;this.notice('Не хватает маны: '+Object.entries(c).filter(([k,v])=>p.mana[k]<v).map(([k,v])=>K.view(this.s.knowledge,MANA.find(m=>m.id===k)).name+' '+Math.ceil(v-p.mana[k])).join(', '));this.emit('sound',{name:'dry'});return false;}
...
else if(spell.form==='burst'){this.area(p.x,p.y,125*range,spell,27*power,this.wallCenter(target,'burst',range));this.ring(p.x,p.y,125*range,color);this.burstParticles(p.x,p.y,color,35,170);}
...
executeSelf(spell,target,power,range){for(const node of S.compile(spell).nodes){if(node.kind==='aspect')this.selfEffect({element:node.element,form:'self',mod:spell.mod},target,power*node.weight,range);else this.reactionSelf(node,spell,power,range,target);}}
...
transformNode(x,y,r,node,aim){if(node.id==='barrier'){this.placeWall(aim||{x,y});return;}const tx=Math.floor(x/TILE),ty=Math.floor(y/TILE),n=Math.ceil(r/TILE);let changed=false;
...
area(x,y,r,spell,damage,aim){this.transform(x,y,r*.7,spell,aim);for(const e of [...this.s.enemies])if(e.hp>0&&Math.hypot(e.x-x,e.y-y)<r)this.hit(e,damage,spell,x,y);const b=this.s.boss;if(b&&b.hp>0&&Math.hypot(b.x-x,b.y-y)<r+25)this.hit(b,damage,spell,x,y);}
...
for(const b of [...this.projectiles].reverse()){if(!this.projectiles.includes(b))continue;b.life-=dt;const collision=this.traceWall(b.x,b.y,b.x+b.vx*dt,b.y+b.vy*dt);b.x=collision?collision.x:b.x+b.vx*dt;b.y=collision?collision.y:b.y+b.vy*dt;let remove=b.life<=0||!!collision;if(collision&&b.enemy&&this.s.walls[collision.index])this.strikeWall(collision.index,{element:'earth',weight:b.damage/22});
if(b.enemy){if(!collision&&Math.hypot(b.x-p.x,b.y-p.y)<b.r+12){this.hurt(b.damage,b.attacker);remove=true;}}
...
for(let i=this.fields.length-1;i>=0;i--){const f=this.fields[i];f.life-=dt;f.tick-=dt;if(f.tick<=0){f.tick=.65;this.area(f.x,f.y,f.r,f.spell,10*f.power,f.built?false:undefined);f.built=true;this.burstParticles(f.x+(this.random()-.5)*f.r,f.y+(this.random()-.5)*f.r,f.color,3,20);}if(f.life<=0)this.fields.splice(i,1);}
