(function(root,factory){if(typeof module==='object'&&module.exports)module.exports=factory(require('./knowledge.js'),require('./spellcraft.js'));else root.Arcana=factory(root.Knowledge,root.Spellcraft);})(globalThis,function(K,S){
'use strict';
...
return tiles;}
function createState(seed=31719,difficulty='normal'){return {version:3,terrainTimers:{},knowledge:K.initial(),worldPopulated:false,eventLedger:{},terrainCredit:{},nextEnemyId:1,seed,difficulty,tiles:makeWorld(seed),time:0,player:{x:52.5*TILE,y:53.5*TILE,hp:100,maxHp:100,mana:Object.fromEntries(MANA.map(m=>[m.id,K.BASIC.includes(m.id)?24:0])),cap:60,shield:0},spells:[{form:'bolt',element:'earth',mod:'plain'},{form:'bolt',element:'water',mod:'plain'},{form:'burst',element:'air',mod:'plain'},{form:'self',element:'earth',mod:'plain'}].map(S.normalize),selected:0,shrines:SHRINES.map(s=>({...s,status:'dormant'})),relics:[],shards:0,research:0,enemies:[],pickups:[],discovered:new Array(SIZE*SIZE).fill(0),stats:{kills:0,casts:0,transforms:0,deaths:0,collected:0},checkpoint:{x:52.5*TILE,y:53.5*TILE},tutorial:0,boss:null,won:false,started:true};}
...
if(!input||![1,2,3].includes(input.version))return null;
...
return {SIZE,TILE,MANA,FORMS,MODS,EFFECTS,TERRAINS,SHRINES,RELICS,rng,clamp,dist,cost,spellName,canPay,pay,terrainReaction,biome,makeWorld,createState,validSave,migrateSave,spellSequence:S.sequence,normalizeSpell:S.normalize,compileSpell:S.compile,validSpell:S.valid,manaRates,reachable};
});
