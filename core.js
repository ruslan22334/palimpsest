...
return tiles;}
function createState(seed=31719,difficulty='normal'){return {version:6,ecology:{populated:false,anomalies:[]},walls:{},terrainTimers:{},knowledge:K.initial(),worldPopulated:false,eventLedger:{},terrainCredit:{},nextEnemyId:1,seed,difficulty,tiles:makeWorld(seed),time:0,player:{x:52.5*TILE,y:53.5*TILE,hp:100,maxHp:100,mana:Object.fromEntries(MANA.map(m=>[m.id,K.BASIC.includes(m.id)?24:0])),cap:60,shield:0},spells:[{form:'bolt',element:'earth',mod:'plain'},{form:'bolt',element:'water',mod:'plain'},{form:'burst',element:'air',mod:'plain'},{form:'self',element:'earth',mod:'plain'}].map(S.normalize),selected:0,shrines:SHRINES.map(s=>({...s,status:'dormant'})),relics:[],shards:0,research:0,enemies:[],pickups:[],discovered:new Array(SIZE*SIZE).fill(0),stats:{kills:0,casts:0,transforms:0,deaths:0,collected:0},checkpoint:{x:52.5*TILE,y:53.5*TILE},tutorial:0,boss:null,won:false,started:true};}
function validEnemy(e){return !!(e&&Number.isSafeInteger(e.id)&&e.id>0&&Number.isFinite(e.hp)&&e.hp>=0&&Number.isFinite(e.maxHp)&&e.maxHp>0&&Number.isFinite(e.x)&&Number.isFinite(e.y)&&(e.kind!=='leech'||['water','life'].includes(e.drainElement)));}
...
if(!input||![1,2,3,4,5,6].includes(input.version))return null;
...
return {ecology:E,fortifications:F,daylight:D,SIZE,TILE,MANA,FORMS,MODS,EFFECTS,TERRAINS,SHRINES,RELICS,rng,clamp,dist,cost,spellName,canPay,pay,terrainReaction,biome,makeWorld,createState,validSave,migrateSave,spellSequence:S.sequence,normalizeSpell:S.normalize,compileSpell:S.compile,validSpell:S.valid,manaRates,reachable};
});
