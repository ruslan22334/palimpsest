(function(root,factory){if(typeof module==='object'&&module.exports)module.exports=factory(require('./knowledge.js'),require('./spellcraft.js'),require('./daylight.js'),require('./fortifications.js'),require('./ecology.js'),require('./mages.js'));else root.Arcana=factory(root.Knowledge,root.Spellcraft,root.Daylight,root.Fortifications,root.ManaEcology,root.MageRules);})(globalThis,function(K,S,D,F,E,G){
'use strict';
...
{id:'light',name:'Свет',color:'#f0d995',glyph:'☼',source:'Собирайте прямой солнечный свет днём.'},
{id:'shadow',name:'Тень',color:'#ba9cdb',glyph:'☾',source:'Ищите отбрасываемую тень днём, а ночью — разломы.'},
...
const cost=S.cost;
function spellName(s){return S.sequence(s).map(e=>MANA.find(m=>m.id===e).name).join(' → ')+' · '+FORMS.find(f=>f.id===s.form).name;}
function canPay(mana,c){return !!c&&Object.entries(c).every(([k,v])=>mana[k]>=v);}
function pay(mana,c){if(!canPay(mana,c))return false;for(const k in c)mana[k]-=c[k];return true;}
function terrainReaction(t,e){
if(e==='fire'&&t==='ice')return 'water';if(e==='earth'&&['ice','mud'].includes(t))return 'soil';if(e==='water'&&t==='mud')return 'water';if(e==='life'&&t==='mud')return 'tree';
...
return tiles;}
function createState(seed=31719,difficulty='normal'){return {version:7,magesPopulated:false,ecology:{populated:false,anomalies:[]},walls:{},terrainTimers:{},knowledge:K.initial(),worldPopulated:false,eventLedger:{},terrainCredit:{},nextEnemyId:1,seed,difficulty,tiles:makeWorld(seed),time:0,player:{x:52.5*TILE,y:53.5*TILE,hp:100,maxHp:100,mana:Object.fromEntries(MANA.map(m=>[m.id,K.BASIC.includes(m.id)?24:0])),cap:60,shield:0},spells:[{form:'bolt',element:'earth',mod:'plain'},{form:'bolt',element:'water',mod:'plain'},{form:'burst',element:'air',mod:'plain'},{form:'self',element:'earth',mod:'plain'}].map(S.normalize),selected:0,shrines:SHRINES.map(s=>({...s,status:'dormant'})),relics:[],shards:0,research:0,enemies:[],pickups:[],discovered:new Array(SIZE*SIZE).fill(0),stats:{kills:0,casts:0,transforms:0,deaths:0,collected:0},checkpoint:{x:52.5*TILE,y:53.5*TILE},tutorial:0,boss:null,won:false,started:true};}
function validEnemy(e){return !!(e&&Number.isSafeInteger(e.id)&&e.id>0&&Number.isFinite(e.hp)&&e.hp>=0&&Number.isFinite(e.maxHp)&&e.maxHp>0&&Number.isFinite(e.x)&&Number.isFinite(e.y)&&(e.kind!=='leech'||['water','life'].includes(e.drainElement))&&(e.kind!=='mage'||G.valid(e)));}
...
if(!input||![1,2,3,4,5,6,7].includes(input.version))return null;
...
if(s.version===2){
 if(!Array.isArray(s.spells)||!s.spells.every(S.valid)||!Array.isArray(s.tiles))return null;
 s.version=3;s.spells=s.spells.map(S.normalize);s.terrainTimers={};
 for(let i=0;i<s.tiles.length;i++)if(s.tiles[i]==='steam'||s.tiles[i]==='fire'&&s.terrainCredit?.[i])s.terrainTimers[i]={terrain:s.tiles[i],left:s.tiles[i]==='steam'?8:25,after:s.tiles[i]==='steam'?'water':'soil'};
}
if(s.version===3){if(!K.valid(s.knowledge,MANA.map(m=>m.id)))return null;s.knowledge.facts=s.knowledge.facts.filter(id=>!K.LEGACY_SOURCES.includes(id));s.version=4;}
if(s.version===4){s.walls={};s.version=5;}
if(s.version===5){s.ecology={populated:false,anomalies:[]};s.version=6;}
if(s.version===6){s.magesPopulated=false;s.version=7;}
if(!validSave(s))return null;s.spells=s.spells.map(S.normalize);return s;
}
function validTimers(t){return !!(t&&typeof t==='object'&&!Array.isArray(t)&&Object.entries(t).every(([i,v])=>/^\d+$/.test(i)&&Number(i)<SIZE*SIZE&&v&&['fire','steam','ice','mud'].includes(v.terrain)&&Number.isFinite(v.left)&&v.left>0&&v.left<=60&&!!TERRAINS[v.after]));}
function validSave(s){return !!(s&&s.version===7&&typeof s.magesPopulated==='boolean'&&E.valid(s.ecology)&&Array.isArray(s.tiles)&&F.valid(s.walls,s.tiles)&&validTimers(s.terrainTimers)&&K.valid(s.knowledge,MANA.map(m=>m.id))&&typeof s.worldPopulated==='boolean'&&Number.isSafeInteger(s.nextEnemyId)&&s.nextEnemyId>0&&validLedger(s.eventLedger)&&s.terrainCredit&&typeof s.terrainCredit==='object'&&!Array.isArray(s.terrainCredit)&&Object.entries(s.terrainCredit).every(([i,v])=>/^\d+$/.test(i)&&Number(i)<SIZE*SIZE&&v===true)&&Array.isArray(s.tiles)&&s.tiles.length===SIZE*SIZE&&s.tiles.every(t=>TERRAINS[t])&&s.player&&Number.isFinite(s.player.x)&&Number.isFinite(s.player.y)&&s.player.x>=TILE&&s.player.x<(SIZE-1)*TILE&&s.player.y>=TILE&&s.player.y<(SIZE-1)*TILE&&Number.isFinite(s.player.hp)&&s.player.hp>0&&Number.isFinite(s.player.cap)&&s.player.cap>=60&&MANA.every(m=>Number.isFinite(s.player.mana?.[m.id])&&s.player.mana[m.id]>=0)&&Array.isArray(s.spells)&&s.spells.length===4&&s.spells.every(S.valid)&&Array.isArray(s.shrines)&&s.shrines.length===5&&s.shrines.every(x=>x&&['dormant','active','cleared','restored'].includes(x.status))&&Array.isArray(s.enemies)&&s.enemies.every(validEnemy)&&Array.isArray(s.pickups)&&Array.isArray(s.discovered)&&s.discovered.length===SIZE*SIZE&&Array.isArray(s.relics)&&s.stats&&Number.isFinite(s.research)&&Number.isFinite(s.time)&&s.time>=0);}
function manaRates(tiles,p,moving,restored=[],time=0,exposure=D.exposure(tiles,p,time)){const tx=Math.floor(p.x/TILE),ty=Math.floor(p.y/TILE),on=tiles[ty*SIZE+tx],near=new Set();for(let dy=-1;dy<=1;dy++)for(let dx=-1;dx<=1;dx++)near.add(tiles[(ty+dy)*SIZE+tx+dx]);const rates=Object.fromEntries(MANA.map(m=>[m.id,0]));if(!moving&&['soil','grass','stone','sand','tree','mud'].includes(on))rates.earth=3.6;if(near.has('water'))rates.water=3;if(near.has('fire')||near.has('lava'))rates.fire=3.4;if(moving)rates.air=2.7;if(near.has('tree'))rates.life=2.8;if(exposure.sun.daylight){if(exposure.shaded)rates.shadow=2.6;else rates.light=3.2*exposure.sun.intensity;}else if(near.has('rift'))rates.shadow=2.6;if(near.has('rift')||near.has('obsidian'))rates.space=2.8;return rates;}
function reachable(tiles,start,end){const seen=new Set([start]),q=[start];for(let i=0;i<q.length;i++){const id=q[i];if(id===end)return true;const x=id%SIZE,y=Math.floor(id/SIZE);for(const [dx,dy] of [[1,0],[-1,0],[0,1],[0,-1]]){const nx=x+dx,ny=y+dy,n=ny*SIZE+nx;if(nx>0&&ny>0&&nx<SIZE-1&&ny<SIZE-1&&!seen.has(n)&&!F.solid(tiles[n])){seen.add(n);q.push(n);}}}return false;}
return {mages:G,ecology:E,fortifications:F,daylight:D,SIZE,TILE,MANA,FORMS,MODS,EFFECTS,TERRAINS,SHRINES,RELICS,rng,clamp,dist,cost,spellName,canPay,pay,terrainReaction,biome,makeWorld,createState,validSave,migrateSave,spellSequence:S.sequence,normalizeSpell:S.normalize,compileSpell:S.compile,validSpell:S.valid,manaRates,reachable};
});
