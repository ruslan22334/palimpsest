...
test('v2 recipes migrate without changing costs or campaign, and v3 rejects corrupt grammar and timers',()=>{
 const s=setup().s;s.version=2;delete s.terrainTimers;s.spells=s.spells.map(({element,form,mod})=>({element,form,mod}));s.shards=17;const next=A.migrateSave(s);assert.ok(next);assert.equal(next.version,5);assert.equal(s.version,2);assert.equal(next.shards,17);for(let i=0;i<4;i++){assert.deepEqual(next.spells[i].elements,[s.spells[i].element]);assert.deepEqual(A.cost(next.spells[i]),A.cost(s.spells[i]));}assert.deepEqual(A.migrateSave(next),next);
